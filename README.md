...
Go toolchain if missing, all recon tools, `PATH` setup, and the
`reconn` command itself (via `pipx`):
...
reconn --list-modules                        # modules + descriptions
reconn -h                                    # full help
python3 reconn.py -d example.com             # without installing
...
```
output/example.com/
  summary.txt                    # short run summary (plain text)
  summary.md                     # short run summary (markdown)
  json/                          # normalized envelopes + run manifest
    manifest.json                # per-module status, counts, errors, durations
    subdomain_enum.json
    dns_enum.json
    ...                          # one {module}.json per module
  raw/                           # verbatim tool output
    subdomain_enum.raw.jsonl
    nmap_scan.raw.xml
    ...
  txt/                           # derived flat lists (feed downstream modules)
    subdomain_enum.hosts.txt
