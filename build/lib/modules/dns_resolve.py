...
def _resolved_hosts(records: list[dict[str, Any]]) -> list[str]:
    hosts = set()
    for record in records:
        host = record.get("host")
        if not host or record.get("status_code") == "NXDOMAIN":
            continue
        if any(record.get(key) for key in RECORD_KEYS) or record.get("status_code") == "NOERROR":
            hosts.add(str(host))
    return sorted(hosts)
...
def run(domain: str, output_dir: str) -> dict[str, Any]:
    ...
    resolved = _resolved_hosts(records)
    write_text(resolved_path, "".join(f"{host}\n" for host in resolved))
    ...
    envelope = build_envelope(
        ...
        errors=errors,
        derived_files=[targets_path.name, resolved_path.name],
        metadata={
